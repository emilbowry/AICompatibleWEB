...
const connectToMongo = async () => {
	try {
		...
		console.error(
			"Failed to connect to MongoDB. The application will exit.",
			error
		);
		process.exit(1);
	}
...
// console.log(getDataUrl("../assets/bw1.jpg"));
const TimelineData = [
	{
		date: "NOV 2022",
		content:
...
const seedDatabase = async () => {
	...
	try {
		...
		if (count === 0) {
			console.log(
				"'timeline' collection is empty. Seeding with initial data..."
			);
			await timelineCollection.insertMany(TimelineData);
			console.log("Database seeded successfully.");
		} else {
			console.log(
				"'timeline' collection already contains data. Skipping seed."
			);
		}
	} catch (error) {
